...
def get_total_store_count(driver):
    total_count_element = driver.find_element_by_xpath(
...
def get_data_from_store_info(store_element):
    text_views = store_element.find_elements_by_class_name(
...
def scroll_down(driver):
    ...
    TouchAction(driver).long_press(x=0, y=window_height).move_to(
        x=0, y=max(window_height-scroll_distance, 0)).release().perform()
